...
contract Lender is Ownable {
    ...
    function getPoolId(
        ...
        //qanswered this is the creation of the poolId ---->
        // checking the poolId according the Keccak256
    ...
    function setPool(Pool calldata p) public returns (bytes32 poolId) {
        ...
        //q what happens if there is taht pool
        //@audit you can create duplicate poolIDs
        poolId = getPoolId(p.lender, p.loanToken, p.collateralToken);

        // you can't change the outstanding loans

        //q what if we are setting a pool for the first time
        ...
        //@audit this is event will always be emitted
    ...
    function borrow(Borrow[] calldata borrows) public {
        //@audit DOS attack.
        //@audit when borrows is equal will always be ignored
    ...
    function repay(uint256[] calldata loanIds) public {
        //@audit gas insuffince we will keep reading from storage
